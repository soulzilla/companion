<template>
    <section id="command">
        ...
        <el-table
            ...
            <el-table-column prop="name" label="Name" min-width="200" sortable>
                <template slot-scope="scope">
                    <router-link class="el-link el-link--default ellipsis-form" :to="{name: 'Show Command', params: {id: scope.row.id}}">
        ...
        <el-dialog
            ...
            <CommandForm :initialForm="formData" @saved="saved" @cancel="formVisible = false" />
        </el-dialog>
    </section>
</template>
...
<script>
    ...
    import {COMMAND_FETCH, COMMAND_OBTAIN} from "../store/types";
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['commands', 'commandsMeta', 'commandsLoading']),
        },
        created() {
            this.fetchData()
        },
        ...
        methods: {
            ...mapActions([COMMAND_FETCH]),
            ...mapMutations([COMMAND_OBTAIN]),
            ...
            fetchData() {
                ...
                this[COMMAND_FETCH](params)
            },
            handleAdd() {
                this.formTitle = 'New Command';
                this.formData = {};
                this.formVisible = true;
            },
            handleEdit(row) {
                this.formTitle = 'Edit Command';
                this.formData = Object.assign({}, row);
                this.formVisible = true;
            },
            handleDelete(row) {
                this.$confirm('This will permanently delete the command. Continue?', 'Warning', {
                    type: 'warning'
                }).then(() => {
                    commandApi.delete(row.id).then((response) => {
                        this.$message({
